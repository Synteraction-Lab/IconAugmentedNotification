...
import com.hci.nip.android.ui.DisplayNotificationActivity;
...
public class MainActivity extends BaseActivity {
    ...
    private void initializeUIListeners() {
        ...
        btnStart.setOnClickListener(v -> {
            Toast.makeText(getApplicationContext(), "Starting Notification Service", Toast.LENGTH_SHORT).show();
            startService(new Intent(getApplicationContext(), NotificationService.class).setAction(NotificationService.ACTION_START_NOTIFICATION_SERVICE));

            startActivity(new Intent(this, DisplayNotificationActivity.class));
        });
        btnStart.setOnFocusChangeListener(((v, hasFocus) -> {
            if (hasFocus) {
        ...
        btnTest.setOnClickListener(v -> {
            startService(new Intent(getApplicationContext(), NotificationService.class).setAction(NotificationService.ACTION_TEST_NOTIFICATION_SERVICE));
        });
        btnTest.setOnFocusChangeListener(((v, hasFocus) -> {
            if (hasFocus) {
        ...
        btnStop.setOnClickListener(v -> {
            // FIXME: bug, close the service when app closes
            Toast.makeText(getApplicationContext(), "Stopping Notification Service", Toast.LENGTH_SHORT).show();
            stopService(new Intent(getApplicationContext(), NotificationService.class).setAction(NotificationService.ACTION_STOP_NOTIFICATION_SERVICE));
        });
        btnStop.setOnFocusChangeListener(((v, hasFocus) -> {
            if (hasFocus) {
